...
class ServerlessPlugin {
  ...
  syncDirectory() {
    const s3Bucket =
      this.serverless.variables.service.custom.s3Bucket;
    const buildFolder =
      this.serverless.variables.service.custom.client
        .distributionFolder;
    const args = [
      's3',
      'sync',
  ...
  async domainInfo() {
    ...
    const output = outputs.find(
      (entry) =>
        entry.OutputKey === 'WebAppCloudFrontDistributionOutput'
    );

    if (output && output.OutputValue) {
      this.serverless.cli.log(
        `Web App Domain: ${output.OutputValue}`
      );
