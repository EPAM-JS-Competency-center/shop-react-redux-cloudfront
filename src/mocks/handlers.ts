...
export const handlers = [
  rest.get(`${API_PATHS.product}/product`, (req, res, ctx) => {
  ...
  rest.get(`${API_PATHS.product}/products`, (req, res, ctx) => {
    return res(
      ctx.status(200),
      ctx.delay(),
      ctx.json<AvailableProduct[]>(availableProducts) //change
    );
  }),
  rest.get(`${API_PATHS.product}/product/:productId`, (req, res, ctx) => {
